...
// FW_SizingControlBar and put there what you need.
...
CSCBArray FW_SizingControlBar::m_arrBars; // static member

IMPLEMENT_DYNAMIC(FW_SizingControlBar, baseCSizingControlBar);

FW_SizingControlBar::FW_SizingControlBar()
{
    m_szMin = CSize(33, 32);
...
BEGIN_MESSAGE_MAP(FW_SizingControlBar, baseCSizingControlBar)
    //{{AFX_MSG_MAP(FW_SizingControlBar)
    ON_WM_CREATE()
    ON_WM_PAINT()
    ON_WM_NCPAINT()
    ON_WM_NCCALCSIZE()
...
BOOL FW_SizingControlBar::Create(LPCTSTR lpszWindowName, CWnd* pParentWnd,
...
/////////////////////////////////////////////////////////////////////////
// FW_SizingControlBar message handlers

int FW_SizingControlBar::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
BOOL FW_SizingControlBar::DestroyWindow() 
{
...
const BOOL FW_SizingControlBar::IsFloating() const
{
    return !IsHorzDocked() && !IsVertDocked();
}

const BOOL FW_SizingControlBar::IsHorzDocked() const
{
    return (m_nDockBarID == AFX_IDW_DOCKBAR_TOP ||
        m_nDockBarID == AFX_IDW_DOCKBAR_BOTTOM);
}

const BOOL FW_SizingControlBar::IsVertDocked() const
{
    return (m_nDockBarID == AFX_IDW_DOCKBAR_LEFT ||
        m_nDockBarID == AFX_IDW_DOCKBAR_RIGHT);
}

const BOOL FW_SizingControlBar::IsSideTracking() const
{
...
CSize FW_SizingControlBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
    ...
    // dirty cast - using FW_SCBDockBar to access protected CDockBar members
    FW_SCBDockBar* pDockBar = (FW_SCBDockBar*) m_pDockBar;
...
CSize FW_SizingControlBar::CalcDynamicLayout(int nLength, DWORD dwMode)
{
...
void FW_SizingControlBar::OnWindowPosChanging(WINDOWPOS FAR* lpwndpos)
{
...
// Mouse Handling
//
void FW_SizingControlBar::OnLButtonDown(UINT nFlags, CPoint point) 
{
    if (m_pDockBar != NULL)
    {
        // start the drag
...
void FW_SizingControlBar::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
    if (m_pDockBar != NULL)
    {
        // toggle docking
...
void FW_SizingControlBar::OnNcLButtonDown(UINT nHitTest, CPoint point) 
{
    if (IsFloating())
    {
...
void FW_SizingControlBar::OnNcLButtonUp(UINT nHitTest, CPoint point) 
{
    if (nHitTest == HTCLOSE)
...
void FW_SizingControlBar::OnRButtonDown(UINT nFlags, CPoint point) 
{
    if (m_bTracking)
        StopTracking();
...
void FW_SizingControlBar::OnMouseMove(UINT nFlags, CPoint point) 
{
    if (m_bTracking)
...
void FW_SizingControlBar::OnCaptureChanged(CWnd *pWnd) 
{
...
void FW_SizingControlBar::OnNcPaint() 
{
...
void FW_SizingControlBar::NcPaintGripper(CDC* pDC, CRect rcClient)
{
...
void FW_SizingControlBar::OnPaint()
{
    // overridden to skip border painting based on clientrect
    CPaintDC dc(this);
}

LRESULT FW_SizingControlBar::OnNcHitTest(CPoint point)
{
    if (IsFloating())
...
void FW_SizingControlBar::OnSettingChange(UINT uFlags, LPCTSTR lpszSection) 
{
...
/////////////////////////////////////////////////////////////////////////
// FW_SizingControlBar implementation helpers

void FW_SizingControlBar::StartTracking(UINT nHitTest)
{
    SetCapture();
    ...
    else
    {
        ...
        FW_SizingControlBar* pBar = arrSCBars[i +
...
void FW_SizingControlBar::StopTracking()
{
...
void FW_SizingControlBar::OnTrackUpdateSize(CPoint& point)
{
    ASSERT(!IsFloating());
...
void FW_SizingControlBar::OnTrackInvertTracker()
{
    ASSERT(m_bTracking);
...
BOOL FW_SizingControlBar::GetEdgeRect(CRect rcWnd, UINT nHitTest,
                                    CRect& rcEdge)
{
    rcEdge = rcWnd;
    if (m_dwSCBStyle & SCBS_SHOWEDGES)
...
UINT FW_SizingControlBar::GetEdgeHTCode(int nEdge)
{
...
void FW_SizingControlBar::GetRowInfo(int& nFirst, int& nLast, int& nThis)
{
...
void FW_SizingControlBar::GetRowSizingBars(CSCBArray& arrSCBars)
{
    arrSCBars.RemoveAll();
    ...
    for (int i = nFirst; i <= nLast; i++)
    {
        ...
        if (FindSizingBar(pBar) >= 0)
            arrSCBars.Add((FW_SizingControlBar*)pBar);
    }
}

const int FW_SizingControlBar::FindSizingBar(CControlBar* pBar) const
{
...
BOOL FW_SizingControlBar::NegociateSpace(int nLengthAvail, BOOL bHorz)
{
...
void FW_SizingControlBar::AlignControlBars()
{
    int nFirst, nLast, nThis;
    GetRowInfo(nFirst, nLast, nThis);
...
void FW_SizingControlBar::OnUpdateCmdUI(CFrameWnd* pTarget, BOOL bDisableIfNoHndler)
{
...
void FW_SizingControlBar::LoadState(LPCTSTR lpszProfileName)
{
    ASSERT_VALID(this);
...
void FW_SizingControlBar::SaveState(LPCTSTR lpszProfileName)
{
...
void FW_SizingControlBar::GlobalLoadState(LPCTSTR lpszProfileName)
{
    for (int i = 0; i < m_arrBars.GetSize(); i++)
        ((FW_SizingControlBar*) m_arrBars[i])->LoadState(lpszProfileName);
}

void FW_SizingControlBar::GlobalSaveState(LPCTSTR lpszProfileName)
{
    for (int i = 0; i < m_arrBars.GetSize(); i++)
        ((FW_SizingControlBar*) m_arrBars[i])->SaveState(lpszProfileName);
}

/////////////////////////////////////////////////////////////////////////
// FW_SCBButton

FW_SCBButton::FW_SCBButton()
{
    bRaised = FALSE;
    bPushed = FALSE;
}

void FW_SCBButton::Paint(CDC* pDC)
{
    CRect rc = GetRect();
